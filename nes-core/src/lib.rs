...
impl Tick for Emulator {
    fn tick(&mut self) -> u8 {
        log::trace!("Emulator clock tick");
        let mut cycles = 0;
        cycles += self.ppu.tick();
        cycles += self.ppu.tick();
        cycles += self.ppu.tick();
        cycles += self.cpu.tick();
        log::trace!("Emulator passed a pahse cycles_spent: {cycles}");
        cycles
    }
}
...
impl Emulator {
    pub fn new(nes_file: &[u8], video_backend: impl VideoBackend + 'static) -> Self {
        ...
        log::trace!("Emulator Instance created");

        Self { cpu, ppu }
    }
    pub fn reset(&mut self) {
        log::trace!("Emulator reset");
        self.cpu.reset();
    }
}
