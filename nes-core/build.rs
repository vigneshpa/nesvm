...
fn generate_opcode_match() {
    println!("cargo:rerun-if-changed=codegen/instructions.json");
    println!("cargo:rerun-if-changed=codegen/cycles.json");

    let mut sink = File::create(get_out_dir("/opcode_match.rs")).unwrap();

    let instructions = File::open("codegen/instructions.json").unwrap();
    let cycles = File::open("codegen/cycles.json").unwrap();

    let instructions = serde_json::from_reader::<_, serde_json::Value>(instructions).unwrap();
    let instructions = instructions.as_object().unwrap();
    let cycles = serde_json::from_reader::<_, serde_json::Value>(cycles).unwrap();
    let cycles = cycles.as_object().unwrap();

    let insmap = instructions.get("map").unwrap().as_object().unwrap();
    ...
    for (ins, details) in insmap {
        ...
        for (mode, opcode) in modes {
            let mode = match instructions.get("modes").unwrap().get(mode) {
